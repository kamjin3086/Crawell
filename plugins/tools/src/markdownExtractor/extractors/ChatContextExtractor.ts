...
export class ChatContextExtractor implements IContentExtractor {
  readonly name = 'ChatContextExtractor';
  readonly priority = 2;

  async canExtract(context: ExtractorContext): Promise<boolean> {
    const { element } = context;
    return element.classList.contains('chat-context') || 
           element.querySelector('.chat-context') !== null;
  }

  async extract(context: ExtractorContext): Promise<ExtractResult> {
    const { element } = context;
    const contextElements = Array.from(element.querySelectorAll('.chat-context'));
    
    if (contextElements.length === 0) {
      return { extracted: false, content: '' };
    }

    const extractedContent = contextElements.map(contextElement => {
      const title = contextElement.querySelector('.context-title')?.textContent?.trim() || '';
      const description = contextElement.querySelector('.context-description')?.textContent?.trim() || '';
      const timestamp = contextElement.querySelector('.timestamp')?.textContent?.trim() || '';
    ...
    return {
      ...
      metadata: {
        contextCount: contextElements.length,
        type: 'context'
      }
    };
  }
